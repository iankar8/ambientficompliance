...
exports.DEFAULT_TRANSITIONS = exports.DEFAULT_STATE_SIGNATURES = exports.WorkflowStateInference = void 0;
exports.generateStateInferenceGuidance = generateStateInferenceGuidance;
/**
 * State inference engine using pattern matching
 */
class WorkflowStateInference {
    stateSignatures;
    transitionPatterns;
    constructor(signatures = exports.DEFAULT_STATE_SIGNATURES, transitions = exports.DEFAULT_TRANSITIONS) {
    ...
    inferState(pageInfo) {
        ...
        for (const signature of this.stateSignatures) {
            ...
            if (signature.indicators.requiredElements) {
                maxScore += 30;
                const foundElements = signature.indicators.requiredElements.filter(el => pageInfo.presentElements.includes(el));
            ...
            if (signature.indicators.requiredText) {
                maxScore += 20;
                const foundText = signature.indicators.requiredText.filter(text => pageInfo.visibleText.toLowerCase().includes(text.toLowerCase()));
            ...
            if (signature.indicators.forbiddenElements) {
                maxScore += 15;
                const foundForbidden = signature.indicators.forbiddenElements.filter(el => pageInfo.presentElements.includes(el));
     ...
     */
    predictNextStates(currentState, availableActions) {
     ...
     */
    validateTransition(from, to, action) {
        return this.transitionPatterns.some(t => t.from === from && t.to === to && t.triggerAction === action);
    }
}
exports.WorkflowStateInference = WorkflowStateInference;
/**
 * Default state signatures for common banking workflows
 */
exports.DEFAULT_STATE_SIGNATURES = [
    {
        name: 'login_page',
        confidence: 0.6,
        indicators: {
 ...
 */
function generateStateInferenceGuidance() {
